...
public class ItemCompatHandler {
    public void onArrowLoose(Player player, ItemStack bow, Level level, int charge, boolean hasAmmo) {
    ...
    public int onItemUseTick(LivingEntity entity, ItemStack item, int duration) {
        ...
        if (item2 instanceof BowItem && duration2 < 20 || item2 instanceof TridentItem && duration2 < 10) {
            ...
            return duration - quickChargeLevel;
        }
        return -1;
    }

    public int onLootingLevel(LivingEntity entity, @Nullable DamageSource damageSource, int lootingLevel) {
        ...
        if (source instanceof AbstractArrow) {
            ...
            if (source instanceof ThrownTrident) {
                ...
                return ModRegistry.ARROW_LOOTING_CAPABILITY.maybeGet(source).map(ArrowLootingCapability::getLevel).orElse((byte) 0);
            }
        }
        return lootingLevel;
    }
